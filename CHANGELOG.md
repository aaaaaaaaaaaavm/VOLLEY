...
Reconciled P37, P46, E29 and P95 against their already-published closing evidence. These are disposition repairs, not new experiments or changed thresholds. Gen5 retains 13 computation items and three decisions. Gen6 retains 18 computation items. Replaced the stale state page with a current handoff and narrowed an unsupported absolute comparison with spring deployers. Historical results and manuscript content are preserved.
...
self-test. There was no test runner and no `tests/`. Every fault injection this repository
has ever run -- nine against the document gate, nine against the freshness gate, four
against the P116 identities, three against the seed-path check -- was written in a
throwaway script, proved the fix on the day, and protected nothing afterwards. That is how
P114 and P116 came to be the same mistake twice.

| ID | Item | Detail |
|---|---|---|
| Tests | `tests/`, run by `verify_all.sh` and by CI | Thirty-one tests in three files, plus a README that says what they do not do. Not a replacement for a run sheet: bands in `validation/` are still the only place a claim is earned against a criterion declared before the script existed |
| Properties | Rules that must hold for ANY input, generated by hypothesis | The thirty-five identities in `host_reference.self_test()` check one altitude and one stage mass. P114 was a claim that is true at 500 km on a 1000 kg stage and false in general, and nothing evaluating only the reference point can tell those apart. Nine orbital properties and nine campaign properties now range over 200 to 2000 km and 400 to 3000 kg: a single impulse always out-raises the same dv spent as a complete transfer, a prograde impulse never moves perigee, the Hohmann inversion round-trips, burn duration scales exactly as one over thrust, every main-engine impulse reaches the floor and every impulse below it is assigned away, each leg starts where the last ended, a propagated campaign always reaches at least the naive multiple, and raising either the burn floor or the thrust never makes a case more executable |
| Regressions | Every shipped defect, reintroduced, with a gate required to catch it | P116 in both directions, the multiplied campaign duration, the campaign that never leaves its start orbit, the accounting that drops the auxiliary half, an edited value inside a generated block, a removed block marker, a one-ulp drift tolerated beside a factor-of-15.6 staleness caught, a flipped band verdict, a removed key, and the dead `$B` path in the issue seeder. Each asserts a failure list is NON-empty, so it fails when the defect stops being caught. A control test asserts the self-test passes when nothing is broken, without which the rest would pass against a self-test that always fails |
...
The first reverses the study's headline conclusion for the second time in two days, and it runs
opposite to the error P114 corrected: that one understated what the engine was asked to do, this
one credited the engine with manoeuvres it cannot perform.

| ID | Item | Detail |
|---|---|---|
| P116 | HIGH. Case B and Case C counted main-engine ignitions for impulses below the file's own minimum burn | Numbered under [ADR-021](docs/adr/021-freeze-the-register.md) rule 2. Section 10 computes that a 2 s burn on the 1000 kg reference stage at 20 kN is a 40.27 m/s command; section 11 then modelled 20 m/s and 40 m/s legs, whose individual impulses are 0.49 s and 0.94 s. Not one of the sixteen reaches the floor. The file published 7 and 11 post-primary ignitions, called the excess over the assumed budget of 4 its sharpest finding, and ranked restart count as the most valuable provider datum. All three followed from charging the engine for manoeuvres its declared envelope forbids. Every impulse is now checked and assigned to `MAIN_ENGINE` or `AUXILIARY_PROPULSION_REQUIRED`, three branches price what can actually be flown, and the hierarchy puts minimum impulse first and restart count second |
| Propagation | Sequential campaign altitude was a multiplication | Each leg was solved from 500 km and the first leg's raise multiplied by the leg count, published as "cumulative raise": 399 km over eleven legs. The same total dv buys more altitude from a higher orbit, so the campaign ends higher than the multiplication says. Propagated leg by leg, eleven legs reach 915.5 km, a net rise of 415.5 km, and the old figure was 3.9 % low. Mass propagates too, which is what makes the minimum commandable step grow along a campaign. Not numbered: an approximation error in an illustration, in the conservative direction, that reversed no conclusion |
| Pacing | Campaign duration was the first leg's arc times the leg count | The transfer ellipse grows with the orbit, so the arcs lengthen. Summed properly, Case C moves from 3.97 to 4.10 h and an eleven-leg campaign from 8.71 to 9.06 h. Not numbered, same reason |
| Branches | Three branches replace one infeasible reading | Branch 1, main engine only: the smallest shell change whose *each* impulse reaches the floor is 150.4 km and 81.9 m/s from 500 km, growing to 182.3 km by the fourth leg as the stage lightens, four steps inside the 120 kg customer allocation, ending at 1163.3 km. Branch 2, main engine plus auxiliary: every reposition impulse falls to auxiliary propulsion and the main engine performs only the 4.41 s disposal burn, leaving a demand of 60 m/s across six impulses for Case B and 200 m/s across ten for Case C, with no auxiliary capability assumed anywhere. Branch 3, hypothetical throttle: 24.5 % of baseline thrust for Case B and 46.8 % for Case C, marked a sensitivity and not a claim that any engine throttles |
| Architecture | The three-way split is now reached from both ends | Section 10 arrives at main engine coarse, auxiliary fine, VOLLEY for release from the fine end. Branch 2 arrives at the same split from the campaign end, independently. Two arguments landing on one architecture is worth more than either alone |
| Provider data | The engine rows are reordered by what actually decides the mission | Minimum stable burn, then minimum thrust and throttle envelope, then full-engine restart count, then vacuum thrust, then Isp, then restart dwell. The first two decide whether the main engine can command the required impulses at all; restart life matters after that. The earlier ranking put restart count first, which followed from the ignition count P116 corrects |
| Self-test | Ten identities added, guarding the corrections in both directions | An impulse marked executable must reach the floor and an impulse below the floor must be assigned away from the engine; leg *n* ends where leg *n+1* starts; net rise is the propagated difference and never below the naive multiple; duration is the sum of the arcs; main-engine and auxiliary counts equal what was assigned and add to twice the leg count; lowering the floor cannot reduce executability and a zero floor admits everything; raising thrust at a fixed floor cannot increase it; branch 1's binding impulse reaches the floor exactly |
| Self-test | The identity count was hardcoded and had gone stale | It printed "25 identities hold" while the function held more. It is now counted from the source at run time, which is the same staleness the results-freshness gate exists to catch, one level up |
| P114 | Superseded in part, not rewritten | Its ignition arithmetic stands: a shell change is two impulses and disposal is a third ignition. The conclusion drawn from it does not, and the entry now says so and points at P116. Retained as the published record |
| P113 | More important, not closed | Stays `PROGRAMME`, `COMPUTATION`, `LIVE`. If a host main engine cannot perform fine shell changes, dividing orbital work between host, auxiliary propulsion and deployer is a harder problem than the file made it look, not an easier one |
| P108 | Untouched | The Gen6 design point stays at 29.009 m/s. Nothing in this pass lowers a VOLLEY requirement, and the assumed 4-ignition budget stays a `VOLLEY_ASSUMPTION` wherever it appears rather than becoming an engine figure |
| Sources | Two retrievals re-attempted, both still blocked | The specific turbopump article and a company-owned ignition post were re-attempted on 2026-08-27. Both return a 403 at this network's own proxy, before the publisher is reached. No row is promoted to `COMPANY_PRIMARY`, every claim keeps its `SEARCH_SUMMARY` mark, and the file now says that S2 is cited by article title against a landing-page URL because the article-level URL could not be confirmed from here |
| CI | The drift step still demanded byte identity of floating-point output | It regenerated `host_reference.json` and ran `git diff --exit-code`, and on a GitHub runner the transfer-arc sums came back one ulp apart: 143.62078713217508 against 143.6207871321751. `pow` and division are not required to be correctly rounded, so that disagreement is irreducible and is the same class as P115. The file was already being regenerated and compared numerically by the freshness gate one step earlier, with a declared tolerance and a reason, so the byte comparison was a second and incompatible standard applied to the same file. It is removed from the drift step in both the workflow and `verify_all.sh`, with the reason written beside it. The generators that remain there emit integers and formatted text, where byte identity is achievable |
...
| A69 | `free_free_first_mode()` picked the rigid-body modes by sign | A free-free beam's two rigid modes are zero; discretisation puts them at 1.1e-4 and 4.5e-4 against 110.5 for the first elastic mode. Keeping the eigenvalues above zero and dropping the first two means a numerically-zero eigenvalue landing a hair below zero on some other LAPACK is discarded, the slice moves by one, and the function returns the SECOND elastic mode at 2.76x. Under a one-ulp perturbation the sign filter moved by 1.76 relative and selection by magnitude by 2.0e-6. Not numbered: this machine and the runner both picked the right mode and 1.6729 Hz is unchanged, so nothing published was wrong |
| Gate | Three leaves carry their own tolerance instead of loosening a file | The free-free mode at 1e-5 relative, and the two `err_pct` residuals at 1e-5 absolute. Those two are 3.4e-7 and 2.3e-6 PERCENT -- differences of numbers agreeing to fifteen digits, with no significant figures to reproduce -- and band 1 tests them at 0.5 %. Nine faults were injected to check the arrangement, including one that substitutes the second elastic mode for the first |
| Gate | The freshness gate left its own output in the working tree | It re-runs a script, compares, and on a pass left the regenerated file in place. On one machine that is byte-identical and invisible; on a runner it is a real difference, and CI's next step -- `git diff --exit-code` after regenerating the generated documents -- failed on a file this gate had written rather than on anything that had drifted. It now restores the committed file after every comparison, pass or fail. A checker that leaves its output behind is a generator |
| Gate | The document gate's coverage is now stated in the document | The markers hold the tables and the tables are 12 per cent of the file. Three further faults were injected at the edge -- a figure, a unit and a verdict word altered inside a sentence rather than a block -- and the gate passes all three. That is the surface P114 lived on: a correct total in a table with a sentence beside it reading that total as one burn. The file now says so instead of leaving the reader to infer the gate covers more than it does |
...
| Freshness | The results-freshness gate compared floating-point JSON byte-for-byte, which no two machines can satisfy | Found the moment CI first ran: `run_a69.py` and `run_a70.py` reproduce byte-identically here and not on a GitHub runner, because both solve eigenvalue and linear systems through numpy and the last bits depend on the BLAS the interpreter was built against. The gate had only ever executed on one machine, so an unachievable claim had gone unnoticed. Values are now compared at 1e-9 relative with everything non-numeric still compared exactly. Not a weakening: P110's staleness was a factor of 15.6, and the new comparison was shown to catch a 2-parts-in-1e9 drift, a flipped band verdict, a 1 % list element, a changed string and a removed key, while tolerating one part in 1e16 |
| A70 | The admissibility sagittas carry about 1e-4 of numerical noise, by construction | Measured when the first tolerance was still too tight to pass on a runner. A69's centreline reproduces to 3e-9, ordinary BLAS sensitivity in a linear solve. A70's sagittas reproduce only to 1.3e-4, and the reason is cancellation: a three-point sagitta subtracts nearly equal ordinates, about 1.627 mm at the 40 mm land, to leave 0.049 um, amplifying relative error by roughly 33,000. 3e-9 times that is 1e-4, which is what is observed. The figure is conditioned badly by construction rather than computed badly, and nothing physical moves: 0.43398 um against 0.43404 um on a 25 um radial clearance, where A70's conclusions are factors of two. Tolerances are now per file with the reason beside each, and the gate still catches staleness by three orders of magnitude. Embedded numbers inside run-sheet detail strings are compared numerically too, so a moved value is tolerated while a changed verdict word is not |
...
| P89 | Closed | `cad/parameters.json` gains a `gen6_seal` group carrying both numbers and saying which is which, `friction_max_N` 17.8 as the specification, `friction_allowance_N` 83.40371375447981 as A41's retained ceiling, 4.68x apart, plus `measured: NO`. |
...
| DISP-01 | 3.9798 % propagated into four documents | `ADR-033`, `docs/GENERATIONS.md`, `docs/BUILD_READINESS.md` and `docs/generations/GEN6.md` all carried A44's 1.113 % as Gen6's figure, 3.6x low. Every surviving instance is now explicitly attributed to A44's 2.18 m machine. Friction's variance share rises with it, 93.4 % to 98.68 %. |
...
| ADR-034 | Adopted: 8.0 m of stroke at 22.73 bar | Exit velocity held at 29.009 m/s (34.280 zero-friction), peak acceleration 25.00 to 11.36 g, gas per shot 112.3 to 51.0 g. Both falls are 54.55 %, from the same charge-pressure change. Added mass per satellite 1.403 to 1.296 kg. |
| P78 | A49 band 6 failed, and it is the finding rather than the failure | Friction scales linearly with contact length while shot work saturates, so friction's share rises across the sweep: 9.75 % to 12.90 % at fixed pressure, and 28.39 % at the adopted point. The gap between the two velocity numerators widens from 5.00 % to 15.38 %. P67 now governs the design point rather than a correction to it. |
...
| P81 | And the backup ejector got 3.67x worse | Clearing the sealed tube cost 181.8 J at 2.18 m against a 4.5 J ejector. At 8.0 m it is 667.2 J, a 148x shortfall. A53's masses are the 2.18 m case and are no longer governing. |
...
| PRED-03 | The first prediction this session that held | Declared before the run: *band 3 fails, everything else passes, fill is roughly 4 s at 1 mm, the correction is 6 L to 8 L.* Fill was 4.14 s, band 3 failed alone, and the isothermal correction is 7.65 L. It is also the first that was checked with a calculation before being written down, three of the four wrong ones were intuitions. |
...
| PRECISION-01 | The precision argument finally lands on a mechanism | Charge pressure gives 0.499 % of velocity per 1 %, and it is a static measurement taken before the shot. A40 measured valve timing at 10.53 % per ms. Commanding on the charge stroke is ~21x more precise than commanding during it. |
| RATIO-01 | The expansion ratio is the binding variable, not the pressure | Velocity saturates toward a 2139 J constant-pressure ceiling, exactly A39's assumption, while gas grows linearly with chamber volume. 2 L to 4 L buys 1.0 m/s and costs 3.2 kg. |
| PRED-02 | Three predictions, three wrong, on a run where every band passed | The seal was predicted to bite: tolerance is 0.83 mbar·L/s against static seals at 10⁻⁶, wrong by four orders. Refined to friction: tolerance 83.4 N, wrong by 4x. Then the gas budget: 4.66 kg against a 12.55 budget, wrong by 2.7x. The first two were caught and corrected before declaring; the third was declared in confidence and failed. |
...
| P62 | The published wiki was sixteen days and four corrections behind its own source | `wiki/Home.md` is tracked and corrected by every propagation here. The live wiki is a different repository that nothing writes to, and nothing said so. It still named the project *emocd*, quoted 20.37 m/s, reported x1.62, and made the withdrawn phase claim, on the most public surface this project has. |
...
| TIPOFF-01 | The closure does not merely survive, it improves | Settling falls 27.88 to 17.69 ms in a 133.3 ms stroke, so it occupies 13 % instead of 19 %. Critical restitution rises 0.9263 to 0.9462 against a published aluminium range of 0.3-0.7. Residual at force removal is still exactly zero for every clearance A23 tabulated. Arrival worsens to 355 °/s and stays what A34 established: transient, never a release rate. |
...
| COST-01 | The cost lands on preload, as predicted | 81.2 to 201.7 N per contact. Band 5 passes at a 250 N limit, and passing it is not the same as the mechanism being easy: A34 already records that the cradle mechanism does not exist, and a 202 N preload releasing inside a 1 N residual is harder than an 81 N one. |
...
| ENV-01 | N = 116 cannot be packaged at all | Every factorisation puts the stack or the width outside the 1500 mm track length, 4 x 29 is a 3016 mm stack, 29 x 4 a 4814 mm width. The largest that fits is N = 126 at 1.941 kg/satellite, on a 244.6 kg machine running a 42-hour campaign. |
...
| WARN-01 | And the flattering metric is named rather than adopted | *Δv per kilogram per satellite* puts this design 5.4x ahead of a dispenser and would rescue the criterion. It is recorded as exactly the sort of metric a project reaches for once the plain one has stopped being kind. |
...
| R5, R6 | What survives, and it is the stronger claim | A clock changes semi-major axis by 0 m at any cadence, and lifetime by x1.0000. One shot changes them by +28.8 km and x1.602. The differentiator is orbit change, and the repository had been leading with the one claim of the two that a clock deletes. |
...
| MOVED-01 | What the corrected point actually changed | Lifetime multiplier x1.62 to x1.60; extension +61.8 % to +60.2 %; ratio against the fastest spring 7.52x to 7.33x and 6.6x to 6.4x on velocity; recoil 65.6 to 64.1 N·s; campaign impulse 0.787 to 0.769 kN·s; cold-gas loss at 3U 7.5x to 8.3x; conjunction minimum 54.9 to 42.2 km; loaded mass 124.5 to 132.5 kg. Every direction is unfavourable, which is the expected sign. |
...
| GATE-01 | What replaces the phase gate: three maturity tiers, each with a stated crossing condition | vault to main: bands declared before the script existed, and run, *unchanged and not negotiable*. main to paper/thesis: stable, effective, reliable. paper/thesis to frozen: published. Nothing crosses upward on enthusiasm, which was the point of the phase gate and survives its retirement. |
| VAULT-01 | `docs/PHASE_II.md` to `docs/VAULT.md`, retargeted in 21 files | Same eighteen entries, renamed to what it is, with the one rule made explicit: every entry states why it stopped. PII-16 carries the edge factor that killed it, PII-17 the mass arithmetic that declined it. That is what makes it evidence rather than a pile. |
| BASE-07 | `BASELINE.md` loses one clause and keeps the rest | *"Performance improvement may not move the baseline"* was a pure freeze artefact and is withdrawn. Naming the trigger, stating which validations it invalidates, propagating scripts to figures to paper, and recording it in an ADR all stay, none of them was ever about phases. Edited in the generator, not the output. |
...
| P46 | The thrust integral is depth-resolved | `thrust_constant()` now Gauss-Legendre averages `B_y` over z ∈ [−45, +45] mm before the Lorentz sum, a change to the physics, not a pasted factor (ADR-015). K_t 11.0258 to 10.5386, ratio 0.9558, exactly A2 band 2's measurement; v_exit 16.388 to 16.029 m/s. `nz = 1` reproduces the superseded value exactly, so A2's ratio stays checkable. Computed and held for three days before being applied. |
| BASE-06 | And it dropped the ceiling below the fleet setpoint | 16.2 m/s against a new ceiling of 16.029 is 101.07 % of it, the exact condition ADR-014 forbids, because a setpoint the machine cannot reach makes the dispersion figure a measure of shortfall rather than of sensing noise. `V_FLEET` 16.2 to 15.8 m/s, holding ADR-014's *fraction* rather than its number. The rule survived contact with the correction. |
| P28 | Regen section 240 to 39 mm | 39 + 300 fits the 339 mm arrest section. Recovery 291 to 47 J. Dropping regen entirely was recommended first and withdrawn: 2 points of efficiency, which is in no kill criterion, against +24 % brake duty, which worsens E34, fourth on the lethality ranking. |
| P10 | Packaging enters the rollup as a labelled placeholder | 8.0 kg, no derivation, named `(P10 PLACEHOLDER, 8.0 kg, no derivation)` so it cannot be cited as computed. Dry 76.5 to 84.5 kg, per 3U 6.378 to 7.042 kg. |
...
| COST-01 | Every headline number moved the wrong way | Efficiency 21.0 to 18.8 %, brake duty 977 to 1162 J, kg per 3U 6.378 to 7.042, lifetime multiplier 1.618 to 1.602. Kill criterion 1 goes from crossed by 3.2x to crossed by 3.5x. Nothing improved. That is what taking these decisions costs, and it is why they were deferred. |
...
| CNT-16 | Register 34 to 31 live | P46, P28, P10, P32 corrected or closed. 10 P-items live, 21 E-items. |
...
| SYN-03 | And together they close kill criterion 2 | Brake run-out 240 to 105 mm. Closed envelope 1839 to 1254 mm at Gen5's own exit velocity, inside ESPA-Grande's ~1270 mm, with 3.9 g of qualification margin still unspent. That criterion has been crossed or unevaluable since the envelope was first drawn, and ADR-023's re-scope could be reversed on evidence rather than defended. |
| SYN-04 | Mass, as a labelled sketch | Scaling `mass_properties.py`'s own lumps to an 850 mm machine with a 0.6 kg shuttle, plus 3 kg of LIM stator iron: dry 76.5 to 59.6 kg, per 3U satellite 6.378 to 4.967 kg. Kill criterion 1 still does not close at 3U and no version of this architecture makes it, but at PocketQube 1P it is 0.207 kg, passing by 10x, and those classes are reachable because the satellite is unmodified. |
...
| A29-01 | The answer, and why it is the comparison that matters | C_d 0.523 on a 201.6 cm² projected frontal area at Re 7.0 x 10⁵ to 1.734 N at exit velocity to 1.127 J over the 1.30 m stroke to deficit 5.116 mm/s (0.0312 %). Small against the design point; 19.2 % of the 3σ dispersion the test exists to resolve. No vacuum chamber needed; an air correction on every measured velocity is. |
...
| A29-03 | The solve does not converge, and that is reported | Velocity residuals fall three orders then plateau near 5 x 10⁻² and oscillate, what a steady solver does on a massively separated wake. The drag is a windowed mean, 1.734 ± 0.144 N (8.3 %). The first attempt read a single iteration, 1.946 N, 12 % high. |
...
| PAPER-10 | §VI-E added; the manuscript is 14 to 15 pages | With `F14_airdrag.png`. Reports the plateau and the viscous bound in the body text rather than in a footnote. Zero undefined references. |
...
| CNT-15 | Register 82 to 83, 12 to 13 corrected | P48. Header propagated. Live count unchanged at 38. |
...
| A2-05 | The first solve of it returned exactly zero and reported success | Residual 150.2 to 4 x 10⁻¹³, clean log, zero field at every sampled point. `gmsh.model.getBoundary()` on the air volume returns the six outer faces *and* all twenty-four magnet, air interfaces; tagging them all as the outer boundary pinned φ = 0 on every magnet surface and the source-free air then gave φ ≡ 0 by uniqueness. Caught by checking the value against physical expectation, not by any tool. The fix selects faces by bounding box and asserts exactly six are found. |
| A2-06 | Periodic endpoint dropped before projection | `getdp`'s `OnLine` includes both ends; x = ±λ/2 are the same point of a periodic field, so keeping both weighted one phase twice. Moves the result 0.084 % to 0.059 %. |
...
| LIC-06 | Sibling repositories relicensed, and two of them not as first planned | `VOLLEY-lab`, `pulsed-linear-motor-design-lab` and `orbital-deployment-trade-study` to CC BY 4.0; `engineering-evidence-toolkit` to Apache-2.0. The two design-lab repositories were originally assigned Apache-2.0 and reassigned on inspection: both carry `reference/volley/motor_model.py`, so a patent-granting licence there would have defeated the reason for the split. |
...
| DOC-02 | Smaller corrections | `CITATION.cff` v0.1.0 to v1.1 with that tag's commit date and `license: CC-BY-4.0`; cogging *torque* to cogging *force* in ADR-004, a linear machine having no torque; SUMMARY's validation count corrected from "nineteen of twenty-one" to twenty-four run sheets. |
...
| ID | Item | Detail |
|---|---|---|
| E30-04 | The delivered-life correction was logged and never propagated | E30-02 established on 2026-08-10 that the headline 7.52x is a ratio of *gains*, and that on delivered orbital life it is 1.495x. That correction went into the register and `REVIEW_RESPONSES.md` and nowhere else, `SUMMARY.md`, `docs/index.html`, `docs/LANDSCAPE.md` and `paper/README.md` all continued to publish the flattering half alone. This is the P42 defect class: a known correction sitting in the register while the public pages say something different. |
| E30-05 | Fixed on all four surfaces, without touching a band | 7.52x is not wrong, A21 band 1 was declared before its run and passed. The fix is to carry 1.495x alongside it, not to delete it, which would be editing a passed band's result. A forward note was added to `A21_comparators.md` that alters nothing below it, following the precedent set when A1's sheet was left as written and its correction placed elsewhere. |
| E30-06 | And the paper is flagged rather than quietly fixed | `paper.tex` still carries 7.5x in the abstract with no delivered-life figure. `paper/README.md` now records that explicitly: the compiled PDF currently quotes the flattering half alone, outstanding for the next build. |
...
| ID | Item | Detail |
|---|---|---|
| E35 | The payload's field exposure is a design variable nobody varied | Review item 16. The payload rides on the sled, 20 mm from the magnets, at 442x magnetometer full scale. At 251 mm it is usable (0.90x); at 400 mm the satellite sees less than Earth's own field. The 251 mm crossing reproduces `PAYLOAD_ENVIRONMENT`'s own figure, which is the cross-check. |
...
| CNT-13 | Register 80 to 81, 37 to 38 live | E35. Header propagated. |
...
| CNT-12 | Register 78 to 80, 35 to 37 live | E33, E34. Header propagated. |
...
| A27-01 | The ball screw is disqualified by kinematics, twice over | 16.388 m/s on a 20 mm lead needs 49,164 rpm. DN = 1.23 x 10⁶ against a 1.5 x 10⁵ ceiling, 8.2x over; whirling critical speed 1,333 rpm, 37x under. The two limits worsen in opposite directions, a fatter screw raises critical speed and worsens DN, so no geometry closes it. |
| A27-02 | The rack fails on vacuum contact, not on speed | Pitch-line velocity is 16.4 m/s against ~10 m/s practice (1.64x over), and torque is a comfortable 69.4 N·m. C4 screens it: a contacting drive at full speed makes E21's open tribology gap the load path rather than an incidental. |
...
| ID | Item | Detail |
|---|---|---|
| SEG-01 | The segmented stator is genuine redundancy, and it was only ever an assertion | `analysis/segment_redundancy.py`. A dead segment is a length the sled coasts over, not a stopped machine: at four segments a later failure still exits at 14.19 m/s, 86.6 % of nominal, and 1.41x a spring. At twelve segments, 95.8 %. |
...
| PAS-01 | Review item 26 answered: neither mechanism is credible in normal use | The 96 V bus is 3.41x below air's 327 V Paschen minimum, and below a gas's minimum nothing breaks down at any pd. Multipaction is the wrong regime by 2.5 x 10⁴, f x d = 40 Hz·m against ~10⁶ Hz·m. Both results are geometry-independent, which is why this is arithmetic. |
...
| CNT-11 | Register 77 to 78, 34 to 35 live | E32. Header propagated. E30 gains the segmentation result and stays LIVE, r is still unmeasured. |
...
| ICD-03 | A qualification gap that is paperwork, not physics | NRCSD-E reportedly requires CubeSats to withstand 0.5-2.5 m/s at ejection *(second-hand, not read in the primary document, and flagged as such)*. VOLLEY is 6.6x that. Physically irrelevant, satellites are damaged by acceleration, not velocity, and 10.53 g sits inside the 25 g CDS cap, but a customer qualified "per NRCSD-E" has no qualification basis for 16.4 m/s. |
...
| CNT-10 | Register 76 to 77, 33 to 34 live | E31. Header propagated. E30 stays LIVE: r is unmeasured, and cycle-life testing is metal, not computation. |
...
| REV-02 | Three answers are losses, stated plainly | A cold-gas module beats VOLLEY at 3U by 7.5x (A21 band 5, declared as a loss *before* the run); the satellite leaves permanently magnetised by remanent magnetisation of soft-magnetic parts; and a payload's magnetometer is unusable inside the deployer at 611x full scale. All three predate the review and come from the project's own analyses. |
| REV-03 | Two of the four judged potentially lethal have nothing behind them | Item 22, a jammed sled forfeits the manifest, not one satellite; `grep -ri jam` returns zero. Item 30, no launch-vehicle ICD has ever been checked for whether it permits separation at 16 m/s, and if one caps near 1-2 m/s the central number is non-compliant by 8x regardless of the physics. Item 30 is also the cheapest question here to answer. |
| REV-04 | A unit correction | Review item 35 cites 18 kW. Peak is 32.5 kW (96 V x 339 A); 16.1 kW is the average over the 158.6 ms stroke. |
...
| E30-01 | The risk/reward ratio, as two numbers | Matching a 0.99-reliable spring on satellites delivered needs per-shot p = 0.9985; matching it on delivered orbital life needs p = 0.9347, because each delivered satellite is worth 1.495x a spring-deployed one. Break-even is p ~ 0.935; below it the spring wins outright. |
| E30-02 | A correction to the project's own headline | The 7.52x lifetime extension is a ratio of *gains* (+61.8 % against +8.2 %). On delivered orbital life it is 1.495x. Both are true, but the second governs any risk-weighted comparison, and 7.5x flatters in exactly the comparison a reviewer makes. |
...
| CNT-09 | Register 75 to 76, 32 to 33 live | E30. Header propagated. |
...
| P46 | Resolving the depth costs 4.42 % of K_t | 11.0258 to 10.5386 N/kA·m, ratio 0.9558 against a declared band of >= 0.95, passes by 0.008. Propagated: v_exit 16.388 to 16.029 m/s, 10.53 to 10.07 g, 339 to 320 A. The baseline is NOT changed; a re-baseline is a change-control action with its own propagation pass, not something a validation does to itself. Computed, reproducible, and held. |
...
| CNT-08 | Register 74 to 75, 31 to 32 live | P46. Header propagated. |
...
| A25-03 | Band 2 passing by 18.8x is itself informative | The rotor is nowhere near a material limit, so the design is not energy-density-constrained. The machine is the binding item, which is where band 4 landed from the other side. |
| E29 | Nothing computes the shot's angular impulse about the host | `astro.py` models the host interaction as one line, `4.0 * DV`, linear only. A13 covers indexing and sled return; the shot's own angular impulse is unmodelled. At a 50 mm CoM miss it is 3.28 N·m·s per shot, 39.3 over a campaign, 2.6x saturation of a 15 N·m·s ESPA-class wheel, saturating around shot four. Bigger wheels and CMGs do not help: the constraint is momentum *storage*. |
...
| CNT-07 | Register 72 to 74, 29 to 31 live | P45, E29. Header propagated. |
...
| ID | Item | Detail |
|---|---|---|
| A24 | The fixed-cell manifest, bands declared at `5fdc978` before the script existed | One cell of 340.5 x 100 x 100 mm on the existing 104 mm pitch, twelve cells, with class-specific inserts subdividing along x. Five of six bands passed. `validation/A24_fixed_cell_manifest.md`, `analysis/cell_manifest.py`. |
| A24-01 | 1U no longer closes kill criterion 1, the rung the repository leaned on | A real insert fits three 100 mm units plus two dividers in a 340.5 mm cell, wasting 37.5 mm: 36 per load and 2.125 kg, not 40 and 1.913. TubeSat is worse, 41 to 24, 1.866 kg to 3.188 kg. Threat 1 still closes, on PocketQube 1P (0.266) and 3P (0.797), two rungs lower, and on the classes with no corner rails and no designed interface at all. |
| A24-02 | Three classes refused outright, which a volume ratio structurally cannot find | ThinSat (114 x 114 x 25.4) exceeds the 100 mm cell section in two axes in every orientation. 12U needs 200 mm in both section axes and there is no second cell in y. 6U survives only as 340 x 100 x 200 across two cells in z. The binding constraint is the 166 mm cassette width, written down nowhere else in this repository. |
| P44 | Band 6 failed: at femtosat scale the separation hardware outweighs the satellites | Declared <= 0.5 % of exit velocity, measured 0.508 %. 720 ChipSats in one cell need 719 shim interfaces, 7.19 kg of hardware to disperse 3.6 kg of payload, and it stops being momentum-neutral because that mass departs with one side. The band was not widened. ChipSat was already 43x outside the mechanism's 200-shot limit; that does not rescue a band declared over every class sharing a cell. |
...
| CNT-06 | Register 71 to 72, 28 to 29 live | P44. Header propagated. |
...
| SITE-02 | And the real validation record | The verification table is rebuilt from A1 through A23: the two outright failures, A12's 37 % finding, A14's 611x band-4 miss, A17's 8.18x amplification, A19 and A20's band-1 failures, and A22/A23's outcomes. Plus the note that twice this month a failed band led to a bug in my own analysis script rather than in the design, which is what declaring a band beforehand is for. |
...
| ID | Item | Detail |
|---|---|---|
| A22 | Retention gates resized, and the fix is eleven grams | 2 x D6 to 2 x D9 A-286. Capacity 18.2 to 41.0 kN, margin at Q = 30 −0.36 to +0.45, positive across Q = 10-30. The design no longer depends on where the unmeasured Q lands, which is the whole point, A19 found Q was the only assumed input moving a margin through zero. Band 1 reproduced A18 band 9 to 0.000 % because the load relation is imported, not reimplemented. |
| A22-01 | No architecture change needed | Splitting the stack across two gates was in the allowed space and is not required, so the magazine is untouched. Quasi-static margin 1.21 to 3.98; pin shear still governs over bearing. D8 misses the 0.20 target by 0.06 and is named because a reader will ask. |
...
| P41 | The payload slams into its cradle at the start of every shot | The 70 mm CoM offset gives a 28.92 N·m moment and 688 rad/s²; the payload crosses its cradle clearance and arrives at 36-231 °/s, 18 to 115x the band. An impact load on the payload's own rails, twelve times per campaign, that appears nowhere in this repository. After impact it rattles, and whether that has settled by release needs a restitution model this project does not have. |
| P41-01 | Tightening the clearance does not fix it | Arrival goes as √(clearance): ten times tighter buys √10, 115 to 36 °/s, still 18x. Preload the cradle instead, A23 band 4 measured the couple reaction it must exceed at 85.0 N per contact. The geometric alternative needs the CoM offset cut 70 mm to 3.5 mm. |
...
| ID | Item | Detail |
|---|---|---|
| OD-00 | Both branches costed before either was chosen | New `analysis/owner_decisions.py` to `results/owner_decisions.json`. It re-runs the real pipeline rather than reasoning about it, and adopts nothing, no branch is written to the baseline. `shot()` gains an `energised` parameter defaulting to `ACCEL_ZONE`, so the alternative is priceable without editing the model; `make_baseline.py --check` confirms the default moves nothing. |
...
| OD-04 | The alternative was priced and rejected | Overhead that is not acceleration zone is 539 mm and does not shrink, so fitting 1270 mm means a 731 mm accel zone. Velocity goes as √s: 16.388 to 12.286 m/s, −25 %, payload KE 537 to 302 J, lifetime x1.62 to x1.44. Repackaging 150 mm recovers about a third and depends on a brake layout nobody has drawn, against an arrest section P28 already calls oversubscribed. |
...
| CNT-04 | Register | 29 live to 27 live; 31 closed. P29 and P9 both closed by decision. |
...
| ADR21-01 | What the discipline bought, stated first | It caught a separation figure wrong by 5.7x, an inter-array force 37 % high, an invariance claim in the paper's own abstract, and P38. It is not overhead and freezing it is not a repudiation. |
...
| CNT-03 | Register counts | 66 / 28 live to 67 / 29 live (15 P, 14 E), 9 `CORRECTED`, 29 `CLOSED`, propagated to all five places that quote them. |
...
| A19-06 | Limitation found on the first run: one-at-a-time cannot see interactions | Fin emissivity returns exactly zero on every output including its own binding quantity, because at the nominal 500 W/m²K contact conductance the joint sinks the heat. That is true at the nominal point and false generally, at 100 W/m²K the fin residual moves 0.1701 to 0.0746 K across the emissivity range. A18 swept the two as a grid and would have caught it; A19 sweeps one at a time and does not. Where two uncertain inputs gate each other, this ranking understates the one currently masked. |
...
| P14-02 | The one that matters is filed in the wrong category | G3-D4, one stator layer or two, is x2 force for the same sheet current against x2 copper mass, has never been computed, and sits upstream of K<sub>t</sub> and the headline velocity. It is filed as a CAD defect and is really an unmade design decision. |
...
| CNT-02 | Register counts | 66 / 33 live to 66 / 28 live (14 P, 14 E), 9 `CORRECTED`, 29 `CLOSED`. Derived from `tools/register_status.py` and propagated to `KILL_CRITERIA.md`, `ROADMAP.md`, `PHASE_I_CLOSURE.md` and the `OPEN_PROBLEMS.md` header, including a fifth counts table in that header which the previous propagation had missed. |
...
| CNT-01 | Register counts | 65 entries / 32 live to 66 / 33 (17 P live, 16 E live), derived from `tools/register_status.py` and propagated to `KILL_CRITERIA.md`, `ROADMAP.md`, `PHASE_I_CLOSURE.md` and the `OPEN_PROBLEMS.md` header. |
...
| A15B8-04 | The trade shape is worse than the ceiling alone suggests | One degree costs the host 8.1x the entire VOLLEY shot. Case B is not a capability the deployer adds to a host; it is one the host would have to already possess, at a scale that makes the deployer's contribution irrelevant. `KILL_CRITERIA.md` §7 carried "plane change 0.12°, effectively nil" as an estimate; it is now a propagated result and a closed-form one that agree, and the row can be read as settled rather than pending. |
| A15B8-05 | Band 7 is still open, and is not being folded in quietly | Band 7 asks whether the campaign spans exactly 12 x 1200 s. That is a property of `build_poem_campaign.py`'s shot scheduling, not a GMAT output, so no propagator run will ever produce it. It closes by reading the generator against ADR-020, which has not been done. Recorded as not evaluated in both the results table and `PHASE_I_CLOSURE.md`. |
...
| FIX-04 | The three routes, costed | Accept the 18.1 s floor: free if the ConOps cadence is >= 18.1 s, and free thirty times over at `astro.py`'s own 1200 s. Raise host control authority: 0.5 N·m to 10.2 s floor, but it is a fifth item on a four-item interface spec and narrows the host set the generic-interface positioning depends on. Counter-mass: removes the momentum at source and costs ~9.4 kg, taking 6.41 kg/satellite to 7.2 on a design already failing kill criterion 1. |
...
| A12-02 | `sizing.py` adopts 2686.6 N under a rule declared before the run | Attraction 3.68 to 2.69 kN, plate stress 33 to 24 MPa, margin 20.2 to 28.1. A4 is not re-run: it was loaded 37 % heavy, so it is conservative and its verdict stands, and repeating a passing analysis at a lighter load would replace a real result with a weaker one. Its JSON records the overload. |
