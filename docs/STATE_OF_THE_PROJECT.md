...
| **P41** | The payload **slams into its cradle at the start of every shot** at 36–231 °/s — 18 to 115× the 2 °/s tip-off comparator. Whether it has settled by release needs a restitution model this project does not have | High |
...
| **0.2** | **Qualify the abstract's three unqualified claims** to match what §V-E and §VII already concede: "unmodified" → mechanically and electrically; "compatible with any stage" → against an interface, envelope pending; add the shared-failure exposure already in §VIII | Three of the seven contradictions above | One editing pass, no new analysis |
...
configuration that fails its own mass threshold by 3×. Then **D5** (the provisional, which has a
