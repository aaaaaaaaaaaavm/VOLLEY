...
A cold-gas module wins by 7.5x at 3U. `validation/A21_comparators.md` band 5 declared this
...
VOLLEY is 6.4x that ceiling. Physically irrelevant, a satellite is damaged by acceleration,
...
| Standoff | Peak field | x Earth's field | x magnetometer full scale |
|---:|---:|---:|---:|
| 20 mm | 61.081 mT | 1357x | 611x |

A payload carrying an attitude magnetometer cannot use it inside this deployer, and
magnetorquers cannot be commanded meaningfully against a 611x bias. A14 band 4 falsified an
...
| 11 | Residual dipole to secular torque on the host | ANSWERED 2026-08-10, E33. Ideal Halbach cancels exactly; tolerance leaves 0.77-1.92 A·m², saturating a 15 N·m·s wheel in 3-7.5 days with the machine idle | Answered, and severe |
...
| 16 | Would separating the mover from the payload sled reduce field exposure and simplify the sled? | PII-11's side-rail layout and PII-14 both touch it. Neither computes the field-exposure benefit, which is the point of the question given 611x | High, it may be the cheapest fix to 7 and 9 |
...
- Matching it on delivered orbital life needs p = 0.9347, because each delivered
  satellite is worth 1.495x a spring-deployed one.
...
> A correction this analysis forces on the project's own headline. The 7.33x lifetime
> extension figure is a ratio of *gains* (+60.2 % against +8.2 %). On delivered orbital
> life the ratio is 1.495x. Both are true, but the second governs any risk-weighted
> comparison, a satellite never released delivers nothing, and the 7.3x figure flatters in
...
comparable to the RF half-period. The converter gives f x d = 40 Hz·m against a lowest
relevant threshold of ~10⁶ Hz·m (1 GHz·mm), a ratio of 2.5 x 10⁴. At 40 kHz the half-period
...
|---:|---:|---:|---|
| 1 µs | 3.73 x 10⁸ A/s | 7,351 V | exceeds |
| 10 µs | 3.73 x 10⁷ A/s | 735 V | exceeds |
| 100 µs | 3.73 x 10⁶ A/s | 74 V | below |
...
| Candidate | Verdict |
|---|---|
| Ball screw | Disqualified by kinematics, twice. 16.029 m/s on a 20 mm lead needs 49,164 rpm: DN = 1.23 x 10⁶ against a 1.5 x 10⁵ ceiling (8.2x over), and whirling critical speed for a 25 mm screw over 1500 mm is 1,333 rpm, 37x under. The two limits worsen in opposite directions, so no geometry closes it |
| Rack and pinion | Pitch-line velocity is 16.0 m/s against ~10 m/s practice (1.64x over), and it carries the drive load through tooth contact at full speed in vacuum, making E21's open tribology gap the load path |
...
Taken in dependency order rather than list order: 16 to 14 to 15 to 19 to 5. Item 16 attacks a
...
|---:|---:|---:|---:|
| 20 mm, where the payload sits today | 44.2 mT | 442x | 983x |
| 100 mm | 306 µT | 3.1x | 6.8x |
| 251 mm | 90 µT | 0.90x, usable | 2.0x |
| 400 mm | 23.7 µT | 0.24x | 0.53x, below Earth's field |
...
- A24, the payload ladder closes kill criterion 1 only at PocketQube class, two rungs
  ...
  designed interface;
- A21, at 3U a cold-gas module wins on mass by 8.3x;
...
8.3x (28), the satellite leaves permanently magnetised (9), and a payload's magnetometer
