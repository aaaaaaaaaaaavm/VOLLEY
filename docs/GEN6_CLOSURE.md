...
| [A52](../validation/A52_gen6_recoil.md) | recoil and angular impulse | 7 of 7. 116.03 N·s/shot, 1.81x Gen5. Answered E29 and closed a NEEDS SOURCE row: the thrust line must pass within 10.7 mm of the host centre of mass |
...
| ~~A57~~ | ~~Attitude rate and packaging on the stage~~, RUN 2026-08-22, seven of eight, band 6 fails as declared, opened P99 and P100 | Both rows close. Attitude rate is *answered*: 0.0112° per shot at 300 kg, 0.135° over an uncorrected campaign, at A52's 10.65 mm alignment requirement. Envelope closes as a measured 200 mm miss costing 1.2579 % of exit velocity if the stroke absorbs it. *The run first used a lever arm 15.6x that requirement, P100, and no band verdict moved when it was corrected* |
...
| ~~superseded~~ | [A54](../validation/A54_pulse_chain.md) fired ADR-033's falsifier, and it was never a consequence of the stroke, the store is sized by power, which the stroke does not enter. Two routes: a store technology at ESR x C <= 36.3 ms against an EDLC's 690-1100 (one datasheet decides it), or withdrawal, which deletes the commanded-velocity claim. P86 | Gen6 has no working correction, and the product's central claim is unsupported |
...
section was 3.57x under-authority and is resized 39.7 to 144.01 mm, and `precharged.py` now
...
shot reaction by 45x and needs seven supports at 1.0 m spacing, which cost 99.7 g, the
...
barely enters it. The run first said Gen6 was 2.33x worse than Gen5 per shot and, at each
