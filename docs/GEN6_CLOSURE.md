...
| **What replaces the trim stage** | **[A54](../validation/A54_pulse_chain.md) fired ADR-033's falsifier**, and it was never a consequence of the stroke — the store is sized by power, which the stroke does not enter. Two routes: a store technology at **ESR × C ≤ 36.3 ms** against an EDLC's 690–1100 (**one datasheet decides it**), or **withdrawal** — which deletes the commanded-velocity claim. **P86** | Gen6 has no working correction, and the product's central claim is unsupported |
