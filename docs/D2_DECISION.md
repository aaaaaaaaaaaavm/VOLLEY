...
| 3U CubeSat | 1 | 12 | 10.550 | crosses, 5.3x | 16.0 m/s |
...
|---|---|---|
| Kill criterion 1 | crossed, 5.3x | closes, 0.440 kg at 1P |
| Commanded velocity per satellite | kept. Cell = satellite | lost. 24 satellites share a shot at 1P, 8 at 3P |
| Against a canisterised dispenser | 1.758x heavier per satellite ([P69](../OPEN_PROBLEMS.md)) | *no comparator run exists at this class* |
...
1. Is the product a velocity or a ride? If the commanded per-satellite differential is the
   product, 3U is the only class that delivers it and kill criterion 1 stays crossed at 5.3x
...
specification nobody defends and the front page keeps a dry mass 6.3x the placeholder it
replaced.
