...
`NEEDS SOURCE` rather than guessing: *specific energy and ESR x C of a film or pulse capacitor.*
...
[A64](../validation/A64_pulse_store_technology.md), six of six: the store is ~70 g, 522x lighter,
...
force to exit velocity. The chain that decides whether the product works is contact state to
lateral impulse and torque to release pose and rate, and it is absent.
