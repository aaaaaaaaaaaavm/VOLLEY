...
| `TubeWall` | **1.0** | mm | `gen6_drive.tube_wall_mm` | A59 band 1, 13.9× margin | **FIXED** |
...
Box **380.5 × 166 × 690**, shelled **−4 mm** from the +Z face. Pattern the **payload cell** at
`CellPitchZ` = 104 mm, six per cassette, **two cassettes**. Cell **340.5 × 100 × 100**.

### 4.6 `Stage_Rail_REFERENCE`

Box **8200 × 120 × 40**, top face at **z = −60**, spanning **x = −100 … 8100**.
...
| `VOLLEY_Chamber_Gen6.step` | 126/120 ⌀ × 176.839, 3 mm caps | Yes | **Provisional envelope** | Import as **reference body**, then remodel when a vessel calc exists |
| `VOLLEY_Reservoir_Gen6.step` | 192/180 ⌀ × 135.969, 6 mm caps | Yes | **Provisional envelope** | Reference body |
| `VOLLEY_Carriage_Gen6.step` | 12 mm disc + seat + 2 stops | Yes | **PLACEHOLDER** | **Do not import.** §5 |
| `VOLLEY_Magazine_Cassette_Gen6.step` | 380.5×166×690 shelled | Yes | Provisional | Remodel natively — you will pattern cells into it |
| `VOLLEY_Stage_Rail_Gen6.step` | 8200×120×40 box | Yes | **Host reference, invented** | Reference body, suppressed from mass |
| `VOLLEY_Trim_Stator_Gen6.step` | ⌀29.805/17.805 × 144.01 belt | Yes | **SUSPENDED** | Leave out; build last if P67 requires it |
...
| Cassette→bore transition | **C** | not started |
...
| **`stage_rail()`** | A 120 × 40 box **invented to have something to check against**. No provider has agreed to anything |
...
7. **Do not invent:** land length, gland cross-section, seal squeeze, valve envelopes, support
   bracket geometry, cassette→bore transition, the offset between `PL_CHAMBER_FACE` and
